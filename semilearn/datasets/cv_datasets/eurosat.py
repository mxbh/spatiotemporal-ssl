...
def get_eurosat(args, alg, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    metadata_file = args.metadata_file if hasattr(args, 'metadata_file') else None

    train_labeled_dataset = EuroSat(alg, data_dir, split="trainval", idx_list=train_labeled_idxs, transform=transform_weak, transform_strong=transform_strong, metadata_file=metadata_file)
    train_unlabeled_dataset = EuroSat(alg, data_dir, split="trainval", is_ulb=True, idx_list=train_unlabeled_idxs, transform=transform_weak, transform_strong=transform_strong, metadata_file=metadata_file)
    val_dataset = EuroSat(alg, data_dir, split="test", transform=transform_val, metadata_file=metadata_file)
...
class EuroSat(ImageFolder, BasicDataset):
    ...
    def __init__(self, alg, root, split, is_ulb=False, idx_list=None, transform=None, target_transform=None, transform_strong=None, metadata_file=None):
        ...
        if metadata_file is not None:
            import yaml
            with open(os.path.join('./data', metadata_file), 'r') as f: 
    ...
    def __getitem__(self, index):
        sample_dict = BasicDataset.__getitem__(self, index)

        if self.metadata is not None:
            idx = self.idx_list[index]
            path, _ = self.samples[idx]
            file = path.split('/')[-1][:-4]
            lng, lat, _ = self.metadata[file]
            sample_dict[f"metainfo_{'ulb' if self.is_ulb else 'lb'}"] = {
                'lat': lat,
                'lng': lng,
                'date': '2000-1-1 00:00:00', # dummy datetime for compatibility
            }
        return sample_dict
