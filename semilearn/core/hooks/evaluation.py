...
class EvaluationHook(Hook):
    ...
    def after_train_step(self, algorithm):
        if self.every_n_iters(algorithm, algorithm.num_eval_iter) or self.is_last_iter(algorithm):
            ...
            if not algorithm.multilabel:
                ...
                if algorithm.log_dict['eval/AP'] > algorithm.best_eval_ap:
                    algorithm.best_eval_ap = algorithm.log_dict['eval/AP']
                    algorithm.best_it = algorithm.it
    ...
    def after_run(self, algorithm):
        ...
        if not algorithm.multilabel:
            ...
            results_dict = {'eval/best_AP': algorithm.best_eval_ap, 'eval/best_it': algorithm.best_it}
        if False: #'test' in algorithm.loader_dict:
            ...
            algorithm.test_dict = algorithm.evaluate('test')
            if not algorithm.multilabel:
                results_dict['test/best_acc'] = algorithm.test_dict['test/top-1-acc']
            else:
                results_dict['test/best_AP'] = algorithm.test_dict['test/AP']
