...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_json_dset, get_pkl_dset, get_bigearthnet
    ...
    # speech dataset
        ...
        lb_dset, ulb_dset, eval_dset, test_dset = get_bigearthnet(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        if args.debug:
            lb_dset.df = lb_dset.df.head(100)
            ulb_dset.df = ulb_dset.df.head(100)
            eval_dset.df = eval_dset.df.head(100)
            test_dset.df = test_dset.df.head(100)
    else:
        return None
