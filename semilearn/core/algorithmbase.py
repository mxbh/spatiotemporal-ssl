...
class AlgorithmBase:
    ...
    def set_hooks(self):
        ...
        self.register_hook(EarlyStoppingHook(), None, "HIGH")
        ...
        if self.args.use_wandb:
            log_key_list = ['train/sup_loss', 'train/unsup_loss', 'train/total_loss', 'train/util_ratio', 
                             'train/run_time', 'train/prefetch_time', 'lr']
            if not self.multilabel:
                log_key_list.extend(['eval/top-1-acc', 'eval/precision', 'eval/recall', 'eval/F1'])
            else:
                log_key_list.extend(['eval/AP', 'eval/AP_micro', 'eval/F1', 'eval/F1_micro'])
            self.register_hook(WANDBHook(log_key_list), None, "LOWEST")
    ...
    def get_save_dict(self):
        ...
        if not self.multilabel:
            save_dict["best_eval_acc"] = self.best_eval_acc
        else:
            save_dict["best_eval_ap"] = self.best_eval_ap
    ...
    def load_model(self, load_path, strict=True):
        ...
        self.ema_model.load_state_dict(checkpoint["ema_model"], strict=strict)
        self.loss_scaler.load_state_dict(checkpoint["loss_scaler"])
        self.it = checkpoint["it"]
        self.start_epoch = checkpoint["epoch"] - 1
        self.epoch = self.start_epoch
        self.best_it = self.it # checkpoint["best_it"]
        if not self.multilabel:
            self.best_eval_acc = checkpoint["best_eval_acc"]
        else:
            self.best_eval_ap = checkpoint["best_eval_ap"]
