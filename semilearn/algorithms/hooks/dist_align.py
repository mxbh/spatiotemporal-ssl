...
class DistAlignEMAHook(Hook):
    ...
    """
    def __init__(self, num_classes, multilabel=False, momentum=0.999, p_target_type='uniform', p_target=None):
        super().__init__()
    ...
    def dist_align(self, algorithm, probs_x_ulb, probs_x_lb=None):
        ...
        if not self.multilabel:
            ...
            probs_x_ulb_1_aligned = probs_x_ulb * (self.p_target + 1e-6) / (self.p_model + 1e-6)
            probs_x_ulb_0_aligned = (1 - probs_x_ulb) * (1 - self.p_target + 1e-6) / (1 - self.p_model + 1e-6)
            probs_x_ulb_aligned = probs_x_ulb_1_aligned / (probs_x_ulb_0_aligned + probs_x_ulb_1_aligned)
    ...
    def set_p_target(self, p_target_type='uniform', p_target=None):
        ...
        if p_target_type == 'uniform':
            p_target = torch.ones((self.num_classes, )) / (self.num_classes if not self.multilabel else 2.)
