...
@ALGORITHMS.register('stssl_defixmatch')
class STSSLDeFixMatch(STSSLAlgorithmBase):
    ...
    def train_step(self, x_lb, x_lb_s, y_lb, x_ulb_w, x_ulb_s, metainfo_lb, metainfo_ulb):
        num_lb = y_lb.shape[0]
        ...
        with self.amp_cm():
            if self.use_cat:
                ...
                metainfo = torch.cat((metainfo_lb, metainfo_lb, metainfo_ulb, metainfo_ulb))
                joint_outputs = self.model.forward_joint(inputs, metainfo)
                logits_x_lb, logits_x_lb_s = joint_outputs['joint_logits'][:2*num_lb].chunk(2)
                logits_x_ulb_w, logits_x_ulb_s = joint_outputs['joint_logits'][2*num_lb:].chunk(2)
                feats_x_lb, feats_x_lb_s = joint_outputs['joint_meta_feat'][:2*num_lb].chunk(2)
                feats_x_ulb_w, feats_x_ulb_s = joint_outputs['joint_meta_feat'][2*num_lb:].chunk(2)
                # visual-only student
                student_inputs = torch.cat((x_lb, x_lb_s, x_ulb_s))
                student_outputs = self.model.forward_student(student_inputs) # no metainfo here
                student_logits_x_lb, student_logits_x_lb_s = student_outputs['student_logits'][:2*num_lb].chunk(2)
                student_logits_x_ulb_s = student_outputs['student_logits'][2*num_lb:]
                student_feats_x_lb, student_feats_x_lb_s = student_outputs['student_meta_feat'][:2*num_lb].chunk(2)
                student_feats_x_ulb_s = student_outputs['student_meta_feat'][2*num_lb:]
            else:
            ...
            student_sup_loss = (1/2) * (self.ce_loss(student_logits_x_lb, y_lb, reduction='mean') + self.ce_loss(student_logits_x_lb_s, y_lb, reduction='mean'))
            ...
            mask_lb = self.call_hook("masking", "MaskingHook", logits_x_ulb=probs_x_lb, compute_prob_x_ulb=False)
            ...
            anti_unsup_loss = self.consistency_loss(logits_x_lb_s,
                                                    ...
                                                    'ce' if not self.multilabel else 'bce',
                                                    mask=mask_lb)
            student_anti_unsup_loss = self.consistency_loss(student_logits_x_lb_s,
                                                            anti_pseudo_label,
                                                            'ce' if not self.multilabel else 'bce',
                                                            mask=mask_lb)
            
            distill_loss = self.distill_loss(
                teacher_feat_lb=torch.cat((feats_x_lb, feats_x_lb_s), dim=0),
                teacher_feat_ulb=feats_x_ulb_s,
                student_feat_lb=torch.cat((student_feats_x_lb, student_feats_x_lb_s), dim=0),
                student_feat_ulb=student_feats_x_ulb_s
            )

            total_loss = sup_loss + self.lambda_u * (unsup_loss - anti_unsup_loss)
            student_total_loss = student_sup_loss + self.lambda_u * (student_unsup_loss - student_anti_unsup_loss) + self.lambda_d * distill_loss
