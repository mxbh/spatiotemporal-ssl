...
def get_config():
    ...
    load_path = args.load_path # save this as it will be overwritten
    ...
    args = parser.parse_args()
    over_write_args_from_file(args, args.c)
    args.load_path = load_path # copy back because we want to test this checkpoint
    #args.num_workers = 0
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    save_path = './tmp/test' #os.path.join(args.save_dir, args.save_name)
    logger_level = "INFO"
    tb_log = None
    ...
    if hasattr(model, "student_model"):
        model.student_model = send_model_cuda(args, model.student_model)
    ...
    if os.path.exists(args.load_path):
        ...
        logger.info("Load path {} does not exist".format(args.load_path))

    # set up ema
    model.ema = EMA(model.model, decay=torch.nan)
    model.ema.load(model.ema_model)

    # START TESTING
    logger.info("Model testing")
    test_dict = model.evaluate('test')
    ...
    # log to json
    log_test_dict_to_json(test_dict, args)
    logger.info(f'Successfully logged test results to json.')   
    
    logging.warning(f"GPU {args.rank} testing is FINISHED")

def log_test_dict_to_json(test_dict, args):
    run_dir = '/'.join(args.load_path.split('/')[:-1])
    log_dict = dict(load_path=args.load_path, **test_dict)
...
if __name__ == "__main__":
    args = get_config()
    port = get_port() if (args.world_size > 1 or args.multiprocessing_distributed) else None
